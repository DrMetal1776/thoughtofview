...
const BATTLE_EXAMPLES = [
  {
    topic: 'Remote Work',
    angleA: { name: 'Hot Take', color: '#e85d3a', emoji: '🔥' },
    angleB: { name: "Devil's Advocate", color: '#7c3aed', emoji: '😈' },
    messages: [
      { side: 'A', text: 'RTO mandates aren\'t about productivity. They\'re about control — and companies know it.' },
      { side: 'B', text: 'Junior employees who started remote are struggling. The office wasn\'t just meetings — it was mentorship.' },
      { side: 'A', text: 'You can\'t schedule osmosis. But you also can\'t justify a 2-hour commute for meetings that could be emails.' },
    ],
    winner: 'Hot Take',
    reason: 'Sharper argument, better examples',
  },
  {
    topic: 'Bitcoin',
    angleA: { name: 'Contrarian', color: '#d97706', emoji: '↩️' },
    angleB: { name: 'Expert Analysis', color: '#059669', emoji: '🎓' },
    messages: [
      { side: 'A', text: 'Bitcoin has no cash flows, no intrinsic value. It\'s a religion, not a currency.' },
      { side: 'B', text: 'Fixed supply plus institutional adoption plus dollar debasement — the math points one direction.' },
      { side: 'A', text: 'Every bubble in history had believers. That\'s the point — belief is the only thing holding it up.' },
    ],
    winner: 'Expert Analysis',
    reason: 'More rigorous framework for the argument',
  },
  {
    topic: 'AI & Jobs',
    angleA: { name: 'Hot Take', color: '#e85d3a', emoji: '🔥' },
    angleB: { name: 'Balanced', color: '#3a7bd5', emoji: '⚖️' },
    messages: [
      { side: 'A', text: 'If your job produces text, code, or analysis — you have 3-5 years. The math is brutal.' },
      { side: 'B', text: 'Every major technology shift created more jobs than it destroyed. AI will be no different.' },
      { side: 'A', text: 'This time the technology compresses the skill gap. That\'s never happened before at this speed.' },
    ],
    winner: 'Hot Take',
    reason: 'The speed argument is genuinely new',
  },
]
...
export default function Home() {
  ...
  useEffect(() => {
    const cardTimer = setInterval(() => setCardIndex(i => (i + 1) % CARD_EXAMPLES.length), 3000)
    const battleTimer = setInterval(() => setBattleIndex(i => (i + 1) % BATTLE_EXAMPLES.length), 4000)
    return () => { clearInterval(cardTimer); clearInterval(battleTimer) }
  }, [])
    ...
    <div className="min-h-screen bg-[#F0EDE6]">
      ...
      <div className="max-w-6xl mx-auto px-4 pb-16 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          {/* Cards quick link */}
          <div className="bg-[#0d1117] rounded-xl p-5 border border-[#1a2a3a]">
            <p className="text-xs font-bold uppercase tracking-widest text-[#4dd9c0] mb-2">🎨 New</p>
            <h4 className="font-serif text-base font-bold text-white mb-2">Take Card Generator</h4>
            <p className="text-xs text-[#8b949e] leading-relaxed mb-4">Turn any take into a shareable 1080×1080 card for Instagram or X.</p>
            <Link href="/cards"
              className="block w-full bg-[#4dd9c0] text-[#0d1117] text-xs font-bold px-4 py-2.5 rounded-lg text-center hover:bg-teal-400 transition-colors uppercase tracking-wider">
              🎨 Create a Card
            </Link>
          </div>
        </div>
